...
class ResolutionNodeMapperTest {
    ...
    @BeforeEach
    void setUp() {
        resolutionNodeMapper = new ResolutionNodeMapper(objectMapper, formatDate, categoryMapper);
    }
    ...
    void shouldMapResolutionObjectNode(final String description) {
        ...
        when(delta.getCategory()).thenReturn("5");
        when(categoryMapper.map(any())).thenReturn(CATEGORY);
